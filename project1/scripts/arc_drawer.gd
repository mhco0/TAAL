...
func draw_circle_arc(center: Vector2, radius: float, angle_from: float, angle_to: float, color: Color):
	...
	var points_by_machine: PoolVector2Array = PoolVector2Array()
	points_arc.push_back(center)
	points_by_machine.push_back(center)
	...
	for i in range(nb_points + 1):
		...
		points_by_machine.push_back(center + Vector2(angle_cos, angle_sin) * radius)
		points_arc.push_back(center + Vector2(cos(angle_point), sin(angle_point)) * radius)
	
	print(points_arc)
	print(points_by_machine)
	#draw_polygon(points_arc, colors)
	draw_polygon(points_by_machine, colors)
